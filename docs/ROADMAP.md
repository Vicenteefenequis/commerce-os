...
- **M12.4 — Link da loja visível no admin** ⏳ planejado: hoje, mesmo com `/loja/[tenantSlug]` existindo (M12.1/M12.5), o dono do negócio não tem onde ver o próprio link dentro do `/admin`. Adicionar esse link (com botão de copiar) a alguma tela do `/admin` (ex: dashboard ou configurações) fecha esse gap operacional. **Fora de escopo, por decisão deliberada do PRD:** como o cliente final descobre esse link (WhatsApp, Instagram, QR na portaria, site do negócio) — isso é "canal de distribuição própria", listado no PRD §6.3 como objetivo de *longo prazo*, e `marketplace` está explicitamente fora do MVP (PRD §7, §R7). A plataforma não se propõe a gerar tráfego/descoberta pro negócio agora — só a deixar o link que já existe fácil de achar e compartilhar.
- **M12.5 — Slug em vez de UUID no link da loja** ✅ concluído: o link que cada tenant divulga (M12.1) era `/loja/<uuid-da-organization>` — não editável, difícil de reconhecer e nada amigável de compartilhar. `Organization` e `Venue` passaram a ter `slug` (único por escopo — global para Organization, por tenant para Venue), gerado automaticamente do nome na criação e editável antes de salvar; a leitura pública do storefront (`GET /storefront/tenants/:tenantSlug/...`, antes `GET /storefront/venues/:tenantId/...`) foi re-chaveada por slug, e `/loja/[tenantId]/[venueId]` virou `/loja/[tenantSlug]/[venueSlug]`. Checkout/pagamento/ticket continuam operando em UUID sem mudança — o slug só resolve para o UUID no servidor. Change: `add-storefront-tenant-landing` (o delta de `storefront/tenant-landing` foi fundido em `storefront/tenant-entry` no archive, por descrever a mesma página do M12.1 já existente).
