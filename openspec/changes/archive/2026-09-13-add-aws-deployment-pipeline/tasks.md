...
- [x] 5.1 Run `scripts/deploy/release.sh <sha>` end-to-end against the freshly provisioned instance and verify the app is reachable over HTTPS at the `sslip.io` hostname with the expected version - closed in a follow-up fix (`worktree-fix-flaky-integration-tests`, commit `e9586e7`): the apparent test flakiness was actually `turbo.json`'s `test` task not declaring `DATABASE_URL`/`APP_DB_PASSWORD`/`OUTBOX_DB_PASSWORD` in its `env` list, so Turbo's strict env mode silently stripped them and integration tests fell back to the default `localhost:5432` - the real, live dev Postgres - instead of an isolated test database. Fixed by declaring those vars on the task; `release.sh <sha>` then ran clean end-to-end (test -> build -> package -> upload -> deploy -> roundtrip) against the real instance
