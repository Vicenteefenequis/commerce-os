...
All admin UI SHALL derive color, typography, spacing, and radius values from a single shared token set rather than component-local hardcoded values.

#### Scenario: Token change propagates everywhere
- **WHEN** a token value (e.g. the primary color) is changed in the shared token set
- **THEN** every component using that token reflects the new value without per-component edits
