...
import { CreateVenueUseCase, ParentOrganizationNotFoundError } from "../application/create-venue.usecase.js";
...
export async function createVenueController(req: Request, trx: Trx): Promise<TxResult> {
  ...
  try {
    const venue = await useCase.execute({ tenantId: identity.tenantId, name: name ?? "" });
    return { status: 201, body: { id: venue.id, tenantId: venue.tenantId, name: venue.name } };
  } catch (err) {
    if (err instanceof InvalidVenueError) {
